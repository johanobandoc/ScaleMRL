...
def run_backward(time_limit=cheetah._DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the Run Backward task."""
    physics = cheetah.Physics.from_xml_string(*get_model_and_assets())
    task = CustomCheetah(goal='run-backward', move_speed=cheetah._RUN_SPEED*0.8, random=random)
...
@cheetah.SUITE.add('custom')
def flip_backward(time_limit=cheetah._DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns the Flip Backward task."""
    physics = Physics.from_xml_string(*get_model_and_assets())
    task = CustomCheetah(goal='flip-backward', move_speed=cheetah._RUN_SPEED*0.8, random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(physics, task, time_limit=time_limit,
                               **environment_kwargs)
...
class CustomCheetah(cheetah.Cheetah):
    ...
    def __init__(self, goal='run-backward', move_speed=0, random=None):
        super().__init__(random)
        self._goal = goal
        self._move_speed = move_speed

    def _run_backward_reward(self, physics):
    ...
    def get_reward(self, physics):
        if self._goal == 'run-backward':
            return self._run_backward_reward(physics)
