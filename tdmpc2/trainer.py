...
class Trainer():
	...
	def __init__(self, cfg, env, agent, buffer, logger):
		...
		assert self.cfg.episode_length >= self._episode_lengths.max().item(), \
			f'[Rank {cfg.rank}] Expected maximum episode length {self.cfg.episode_length} to be >= max of {self._episode_lengths.tolist()}.'
	...
	def eval(self):
		...
		results = {}
		if self.cfg.multitask and self.cfg.rank == 0:
